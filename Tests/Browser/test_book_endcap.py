...
class BookEndcapBrowserTests(unittest.TestCase):
    ...
    def journey(self, dir='ltr', vertical=False, fixed=False, long=False, size=(390,844)):
        ...
        if not fixed:
            page.wait_for_function('!view.renderer.navigationInFlight')
            self.assertFalse(page.evaluate('cap.visible'))
        for _ in range(60):
        ...
        before = page.evaluate('JSON.stringify({index:view.renderer.currentIndex,location:view.lastLocation,sections:book.sections.map(s=>s.id),toc:book.toc,readIDs,relocations:relocations.length})')
        ...
        self.assertEqual(page.evaluate('view.renderer.currentIndex'),1)
        ...
        self.assertEqual(page.evaluate('JSON.stringify({index:view.renderer.currentIndex,location:view.lastLocation,sections:book.sections.map(s=>s.id),toc:book.toc,readIDs,relocations:relocations.length})'),before)
        ...
        page.wait_for_function('!cap.visible && view.renderer.currentIndex===0')
