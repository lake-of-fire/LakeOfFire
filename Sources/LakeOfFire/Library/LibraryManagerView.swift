    ...
    var body: some View {
        ...
        Section("Feed Preview") {
            if let readerFeedEntry = readerFeedEntry {
                ...
                .padding(.horizontal)
                Reader(readerViewModel: readerViewModel, state: $readerState, action: $readerAction, wordTrackingStats: .constant(nil), isPresentingReaderSettings: .constant(false), persistentWebViewID: "library-feed-preview-\(feed.id.uuidString)", bounces: false)
        ...
        .onChange(of: feed.rssContainsFullContent) { rssContainsFullContent in
            print("rssContains changed... \(rssContainsFullContent.description)")
            refresh(forceRefresh: true)
        }
    }
...
struct LibraryCategoriesView: View {
    ...
    @ObservedResults(FeedCategory.self, configuration: LibraryDataManager.configuration, where: { $0.isDeleted == false }) private var categories
