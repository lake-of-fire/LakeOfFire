...
// --- batched rAF helper (coalesces many calls into one requestAnimationFrame) ---
    ...
    return target => new Promise(resolve => {
        ...
        if (!scheduled) {
            ...
            requestAnimationFrame(() => {
                ...
                queue = [];
                scheduled = false;
                for (const [el, res] of q) res(getter(el));
            });
        }
    });
};

// Use batched versions to avoid one rAF per node
...
// Only considers elements, not text nodes which it assumes are contained by some element
const getVisibleRange = async (doc, start, end, mapRect) => {
    ...
    // Now filter asynchronously in batches
    const visibleNodes = [];
    const BATCH_SIZE = 400;           // tune for speed / responsiveness
    for (let i = 0; i < nodes.length; i += BATCH_SIZE) {
        const batch = nodes.slice(i, i + BATCH_SIZE);
        const results = await Promise.all(batch.map(acceptNode));
        for (let j = 0; j < results.length; ++j) {
            if (results[j] === FILTER_ACCEPT) visibleNodes.push(batch[j]);
        }
        // yield so the UI thread can paint
        await new Promise(r => setTimeout(r, 0));
    }
    // we're only interested in the first and last visible nodes
    const from = visibleNodes[0] ?? doc.body
    const to = visibleNodes[visibleNodes.length - 1] ?? from
...
export class Paginator extends HTMLElement {
    ...
    async render() {
        if (!this.#view) return
            this.#view.render(await this.#beforeRender({
                vertical: this.#vertical,
                rtl: this.#rtl,
    ...
    async page() {
        ...
        return Math.floor(((start + end) / 2) / size)
            ...
            const scroll = async () => {
                console.log("#.#scrollTo 9")
                const element = this.#container
                console.log("#.#scrollTo 10")
                const scrollProp = this.scrollProp
                console.log("#.#scrollTo 11")
                const size = await this.size()
                console.log("#.#scrollTo 12")
                const atStart = await this.atStart()
                console.log("#.#scrollTo 13")
                const atEnd = await this.atEnd()
                console.log("#.#scrollTo 14")
                if (element[scrollProp] === offset) {
                    this.#scrollBounds = [offset, atStart ? 0 : size, atEnd ? 0 : size]
                    console.log("#.#scrollTo 15")
