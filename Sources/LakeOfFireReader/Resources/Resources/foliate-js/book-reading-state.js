// Ephemeral native-issued presentation. It never advances a reading epoch,
// infers read evidence from a locator, or manufactures membership from the DOM.
const clone = value => JSON.parse(JSON.stringify(value))
const uuid = () => globalThis.crypto.randomUUID().toLowerCase()
const sameLocation = (a, b) => !!a && !!b
    && a.sectionURL === b.sectionURL && a.isEndPage === b.isEndPage
const scopeKey = scope => scope ? JSON.stringify([
    scope.articleProgressID, scope.articleEpochID, scope.chapterKey, scope.chapterEpochID,
]) : null
const validNullableString = value => value === null || (typeof value === 'string' && value.length > 0)
const validScope = scope => scope && typeof scope.articleProgressID === 'string'
    && validNullableString(scope.articleEpochID) && validNullableString(scope.chapterEpochID)
...
export class BookReadingStateController {
    ...
    constructor({ postMessage, documentStartedAtMs, topWindowURL,
        onState = () => {}, onInvalidate = () => {}, makeRequestID = uuid }) {
    ...
    relocate({ sectionURL = null, isEndPage = false }, { moved = false } = {}) {
        ...
        if (typeof isEndPage !== 'boolean' || (!isEndPage && (typeof sectionURL !== 'string' || !sectionURL))) return false
    ...
    refresh() {
        ...
        try {
            this.postMessage({ requestID, topWindowURL: this.topWindowURL,
    ...
    apply(requestID, result) {
        ...
        if (result.nativeRefresh === true) {
            if (!sameLocation(result.location, this.#location)
                || result.location.locationRevision !== this.#revision) return false
        ...
        if (!state || !context || !Number.isSafeInteger(state.revision) || state.revision <= 0
            || state.revision < this.#lastSnapshotSequence
            || typeof context.contextID !== 'string' || !context.contextID
            || context.isEndPage !== this.#location.isEndPage
            || state.articleProgressID !== context.articleProgressID
            || state.articleEpochID !== context.articleEpochID
            || !validNullableString(state.articleEpochID)
        ...
        } else if (!validScope(state.scope) || scopeKey(state.scope) !== scopeKey(context.scope)
            || state.scope.articleProgressID !== state.articleProgressID
            || state.scope.articleEpochID !== state.articleEpochID) return false
        const previousScope = scopeKey(this.#state?.scope)
        const previousArticle = this.#state?.articleEpochID
        this.#pendingID = null
        this.#lastSnapshotSequence = state.revision
        this.#state = clone(state)
        this.#context = clone(context)
        this.onState(this.state, this.context, {
            passChanged: previousScope !== scopeKey(state.scope) || previousArticle !== state.articleEpochID,
        })
        return true
    }
    captureContext(expected = null) {
        ...
        if (expected && expected.contextID !== this.#context.contextID) throw new Error('The book or chapter changed. Reopen Book Actions.')
        return { ...this.context, locationRevision: this.#revision }
    }
    captureScope(sectionURL) {
        if (this.#closed || !this.#context || this.#location?.isEndPage
    ...
    noteManualReadSnapshot(sequence) {
        ...
        this.#lastSnapshotSequence = Math.max(this.#lastSnapshotSequence, sequence)
        return true
    }
    admitsNavigation(target) {
        if (!this.ready || target.locationRevision !== this.#revision
            || target.articleProgressID !== this.#context.articleProgressID
            || target.articleEpochID !== this.#context.articleEpochID) return false
        if (target.action === 'startBookOver') return true
        return target.action === 'startChapterOver' && !this.#context.isEndPage
            && target.sectionLocation === this.#context.sectionLocation
            && target.chapterEpochID === this.#context.scope?.chapterEpochID
    }
    close() {
