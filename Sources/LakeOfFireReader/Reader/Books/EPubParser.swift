...
struct EPubParser {
    ...
    private static let metadataWhitespace = CharacterSet(
        charactersIn: "\t\n\u{000C}\r "
    )

    /// Parses the EPUB file at the given URL (either a ZIP archive with an epub extension or an unpacked EPUB directory)
    /// and returns a tuple with the book title, author (if found), cover image relative path (if found), and publication date (if found).
    ...
    static func parseMetadataAndCover(from epubURL: URL) throws -> (title: String, author: String?, coverHref: String?, publicationDate: Date?)? {
        ...
        guard let opfRelativePath = parseContainer(containerData) else { return nil }
        let opfData = try source.readEntry(subpath: opfRelativePath)
        guard let (title, coverHref, author, publicationDate) = parseOPF(opfData) else {
            return nil
        }

        let opfDirectory = (opfRelativePath as NSString).deletingLastPathComponent
        let resolvedCoverHref = coverHref.flatMap {
            ReaderPackageEntrySource.resolveSubpath(
                $0,
                relativeTo: opfDirectory
            )
        }
    ...
    /// Parses the W3C date/time profile recommended for EPUB publication dates.
    /// Reduced-precision values are represented by the first instant in their
    /// stated UTC period because the library model stores one `Date` rather than
    /// the original precision.
    private static func parsePublicationDate(_ value: String) -> Date? {
        let fractionalDateTime = ISO8601DateFormatter()
        fractionalDateTime.formatOptions = [
    ...
    /// Parses the container.xml data and selects the same first typed EPUB
    /// package document used by the renderer.
    private static func parseContainer(_ data: Data) -> String? {
        ...
        return ReaderPackageEntrySource.resolveSubpath(
            foundPath,
            relativeTo: ""
        )
    }

    /// Parses the OPF XML data to extract the primary book title, first creator,
    /// cover image href, and publication date. Package elements are matched by
    /// namespace and direct-child position so namespace prefixes and extension
    /// lookalikes cannot change the selected metadata graph.
            ...
            private func finishTextField() {
                ...
                case .title:
                    if !normalized.isEmpty {
                        foundTitle = normalized
                    }
                case .creator:
                    if !normalized.isEmpty {
                        foundAuthor = normalized
                    }
                case .date:
                    if !normalized.isEmpty {
