...
public struct ExternalMediaSubtitlesMessage {
    public struct SubtitleTrack {
        ...
        init?(dictionary: [String: Any]) {
            guard
                ...
                print("Failed to decode SubtitleTrack:", dictionary)
                return nil
            }
        ...
        public var webVTTURL: URL {
            ...
            if let index = queryItems.firstIndex(where: { $0.name == "fmt" }) {
                queryItems[index] = URLQueryItem(name: "fmt", value: "vtt")
            } else {
    ...
    public init?(fromMessage message: WebViewMessage) {
        ...
        if let captionsArray = body["captionsOptions"] as? [[String: Any]] {
            subtitleTracks = captionsArray.compactMap { SubtitleTrack(dictionary: $0) }
        } else {
    ...
    public func preferredTrack(for locale: Locale) -> SubtitleTrack? {
        let localeIdentifier = locale.identifier.lowercased()
        let primaryLanguageCode = locale.languageCode?.lowercased()

        func localeMatchKind(for track: SubtitleTrack) -> Int? {
            let trackCode = track.languageCode.lowercased()
            if trackCode == localeIdentifier {
                return 2
            }
            if trackCode.replacingOccurrences(of: "_", with: "-") == localeIdentifier.replacingOccurrences(of: "_", with: "-") {
                return 2
            }
            if let primaryLanguageCode,
               trackCode.split(separator: "-").first.map(String.init) == primaryLanguageCode {
                return 1
            }
            return nil
        }
        ...
        return subtitleTracks
            .compactMap { track -> (SubtitleTrack, Int)? in
                guard let localeMatchKind = localeMatchKind(for: track) else {
                    return nil
                }
                let exactnessBaseScore = localeMatchKind * 2
                let generationBonus = track.isAutoGenerated ? 0 : 1
                return (track, exactnessBaseScore + generationBonus)
            }
            .sorted {
...
public typealias VideoStatusMessage = ExternalMediaSubtitlesMessage
