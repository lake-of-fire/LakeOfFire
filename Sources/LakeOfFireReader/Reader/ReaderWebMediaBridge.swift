...
public enum ReaderWebMediaBridge {
    ...
    @discardableResult
    @MainActor
    public static func postExternalSubtitlesUpdate(from message: WebViewMessage) -> Bool {
        guard let status = VideoStatusMessage(fromMessage: message),
              let caption = preferredCaptionOption(from: status.captionsOptions),
              let pageURL = status.windowURL
                ?? status.pageURL
                ?? message.mainDocumentURL
                ?? message.requestURL
        else {
            return false
        }
        ...
        postExternalSubtitlesUpdate(
            ReaderExternalMediaSubtitlesUpdate(
                ...
                requestHeaders: subtitleRequestHeaders(from: message, pageURL: pageURL)
            )
        )
        return true
    }

    static func preferredCaptionOption(
        from options: [VideoStatusMessage.CaptionsOption],
        localeIdentifier: String = Locale.current.identifier
    ) -> VideoStatusMessage.CaptionsOption? {
        let normalizedLocale = normalizedLocaleIdentifier(localeIdentifier)
        let preferredLanguage = languageSubtag(normalizedLocale)

        func rank(_ option: VideoStatusMessage.CaptionsOption) -> Int {
            let optionLocale = normalizedLocaleIdentifier(option.languageCode)
            let optionLanguage = languageSubtag(optionLocale)
            let exactLocale = !normalizedLocale.isEmpty && optionLocale == normalizedLocale
            let sameLanguage = !preferredLanguage.isEmpty && optionLanguage == preferredLanguage

            switch (option.isAutoGenerated, exactLocale, sameLanguage) {
            case (false, true, _):
                return 0
        ...
        return options
            .filter { !$0.languageCode.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }
            .min { lhs, rhs in
    ...
    @MainActor
    private static func subtitleRequestHeaders(from message: WebViewMessage, pageURL: URL) -> [String: String] {
    ...
    private static func normalizedLocaleIdentifier(_ rawValue: String) -> String {
        rawValue
    ...
    private static func languageSubtag(_ normalizedIdentifier: String) -> String {
        normalizedIdentifier.split(separator: "-").first.map(String.init) ?? normalizedIdentifier
    }
